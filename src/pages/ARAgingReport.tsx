...
import { useARAgingData, type AgingFilters, type AgingPartnerSummary } from '@/hooks/useAgingReportData';
...
export default function ARAgingReport() {
  ...
  const data = useARAgingData(filters, generated);

  const { data: branches = [] } = useQuery({
    queryKey: ['aging-branches'],
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('business_partners').select('id, card_code, card_name').eq('card_type', 'customer').order('card_name').limit(500);
      return data || [];
    },
  });
  ...
  const pieData = data.bucketLabels.map((label, i) => ({
    name: label, value: data.bucketTotals[i], fill: CHART_COLORS[i % CHART_COLORS.length],
  })).filter(d => d.value > 0);

  const handleExportExcel = useCallback(() => {
    const rows = data.partners.flatMap(p => p.invoices.map(inv => ({
      'Customer Code': p.code, 'Customer Name': p.name,
    ...
    XLSX.writeFile(wb, `AR_Aging_${filters.asOfDate}.xlsx`);
  }, [data, filters]);

  const handleExportPDF = useCallback(() => {
    const doc = new jsPDF('l', 'mm', 'a4');
    doc.setFontSize(16);
    doc.text('Accounts Receivable Aging Report', 14, 15);
    doc.setFontSize(10);
    doc.text(`As of: ${filters.asOfDate}`, 14, 22);
    autoTable(doc, {
      startY: 28,
      head: [['Customer', 'Total Outstanding', ...data.bucketLabels, 'Invoices']],
    ...
    doc.save(`AR_Aging_${filters.asOfDate}.pdf`);
  }, [data, filters]);
    ...
    <div className="container mx-auto p-4 md:p-6 space-y-6 max-w-[1600px]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Analyze customer outstanding balances by aging bucket</p>
        </div>
        {generated && (
            ...
            <CollapsibleContent className="mt-2">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                  ...
                  <Select value={filters.customerId || 'all'} onValueChange={v => setFilters(f => ({ ...f, customerId: v === 'all' ? '' : v }))}>
                    <SelectTrigger><SelectValue placeholder="All" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">All Customers</SelectItem>
                      {customers.map(c => <SelectItem key={c.id} value={c.card_code || c.id}>{c.card_name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
      ...
      {generated && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <KPICard icon={<DollarSign className="h-4 w-4 text-primary" />} label="Total Outstanding" value={`SAR ${formatSARShort(data.totalOutstanding)}`} subtitle={`${data.partners.length} customers`} />
            ...
            <KPICard icon={<TrendingDown className="h-4 w-4 text-chart-4" />} label="Above 90 Days" value={`SAR ${formatSARShort(data.above90Total)}`} subtitle={`${((data.above90Total / (data.totalOutstanding || 1)) * 100).toFixed(0)}% of total`} />
            <KPICard icon={<DollarSign className="h-4 w-4 text-muted-foreground" />} label="DSO (Est.)" value={data.totalOutstanding > 0 ? `${Math.round(data.partners.reduce((s, p) => s + p.invoices.reduce((a, i) => a + i.daysOverdue * i.balanceDue, 0), 0) / data.totalOutstanding)} days` : '0 days'} tooltip="Weighted average days sales outstanding" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm">Top 10 Overdue Customers</CardTitle></CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={260}>
          ...
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm">Customer Aging Detail</CardTitle>
                <div className="relative w-64">
                  <Search className="absolute left-2 top-2.5 h-3.5 w-3.5 text-muted-foreground" />
                  <Input placeholder="Search customer..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-8 h-8 text-xs" />
                </div>
              </div>
            </CardHeader>
                  ...
                  <TableBody>
                    ...
                    {filteredPartners.length > 0 && (
                      <TableRow className="bg-muted/50 font-bold">
                        <TableCell></TableCell>
                        <TableCell>TOTAL ({filteredPartners.length} customers)</TableCell>
      ...
      <PLDrillDownDialog open={drillDown.open} onOpenChange={o => setDrillDown(d => ({ ...d, open: o }))} acctCode={drillDown.acctCode} acctName={drillDown.acctName} companyIds={filters.companyIds} dateFrom="" dateTo={filters.asOfDate} />
    </div>
  );
}

function PartnerRow({ partner, bucketLabels, expanded, onToggle }: {
