...
export default function CustomerGroups() {
  ...
  const { data: groups = [], isLoading } = useQuery({
    queryKey: ['customer_groups', activeCompanyId],
    queryFn: async () => {
      if (!activeCompanyId) return [];
      const { data, error } = await supabase.from('customer_groups').select('*').eq('company_id', activeCompanyId).order('group_code');
  ...
  const saveMutation = useMutation({
    mutationFn: async (row: Partial<Group>) => {
      if (row.id) {
        const { error } = await supabase.from('customer_groups').update({ group_name: row.group_name, notes: row.notes || null }).eq('id', row.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('customer_groups').insert({ company_id: activeCompanyId!, group_name: row.group_name!, notes: row.notes || null });
        if (error) throw error;
      }
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customer_groups'] }); setEditingId(null); setIsAdding(false); setForm({}); toast.success('Saved'); },
    onError: (e: any) => toast.error(e.message),
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from('customer_groups').delete().eq('id', id); if (error) throw error; },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customer_groups'] }); toast.success('Deleted'); },
  });
  ...
  return (
    <div className="space-y-4 p-6 page-enter">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{language === 'ar' ? 'مجموعات العملاء' : 'Customer Groups'}</h1>
          <p className="text-muted-foreground text-sm">Classify customers into groups for reporting and defaults</p>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && groups.length === 0 && !isAdding && (
              <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">No customer groups defined.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
