...
import { Search, Plus, Loader2, PackagePlus, MoreVertical, Eye, Edit, RefreshCw } from 'lucide-react';
...
export default function GoodsReceipt() {
  ...
  const { data: receipts, isLoading } = useQuery({
    queryKey: ['inventoryGoodsReceipts'],
    queryFn: async () => {
  ...
  const filtered = (receipts || []).filter(r =>
    ...
    r.warehouse_code?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6 page-enter">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{language === 'ar' ? 'استلام بضاعة' : 'Goods Receipt'}</h1>
          <p className="text-muted-foreground">{language === 'ar' ? 'استلام بضاعة بدون أمر شراء' : 'Goods receipt without purchase order'}</p>
        </div>
        <div className="flex items-center gap-2">
          <SAPSyncButton entity="inventory_goods_receipt" />
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>{language === 'ar' ? 'استلام بضاعة' : 'Goods Receipts'}</CardTitle>
            </div>
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">{language === 'ar' ? 'لا توجد إيصالات' : 'No goods receipts found.'}</div>
          ) : (
            <div className="border rounded-lg overflow-hidden">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{language === 'ar' ? 'رقم المستند' : 'Doc No.'}</TableHead>
                    ...
                    <TableHead>{language === 'ar' ? 'مستند ERP' : 'ERP Doc'}</TableHead>
                    <TableHead>{language === 'ar' ? 'المزامنة' : 'Sync'}</TableHead>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem><Eye className="mr-2 h-4 w-4" />{language === 'ar' ? 'عرض' : 'View'}</DropdownMenuItem>
                            <DropdownMenuItem><Edit className="mr-2 h-4 w-4" />{language === 'ar' ? 'تعديل' : 'Edit'}</DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => sync('inventory_goods_receipt', 'to_sap', r.id)} disabled={isSyncing}>
                              <RefreshCw className="mr-2 h-4 w-4" />{language === 'ar' ? 'مزامنة إلى SAP' : 'Sync to SAP'}
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => sync('inventory_goods_receipt', 'from_sap', r.id)} disabled={isSyncing}>
                              <RefreshCw className="mr-2 h-4 w-4" />{language === 'ar' ? 'سحب من SAP' : 'Pull from SAP'}
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </TableCell>
